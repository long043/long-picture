...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        // 保存会话到集合中。attributes将数据 ——> session中
    ...
    private void broadcastToPicture(Long pictureId, PictureEditResponseMessage pictureEditResponseMessage, WebSocketSession excludeSession) throws IOException {
        ...
        if (CollUtil.isNotEmpty(sessionSet)) {
            ...
            //由于前端js不能直接接收long类型的数据，会造成精度丢失。所以下面这一段是将
            //创建 ObjectMapper
            ...
            //writeValueAsString(...)：将 pictureEditResponseMessage 转为 JSON 字符串，
            ...
            for (WebSocketSession session : sessionSet) {
                ...
                //给别的客户端发送消息
                if (session.isOpen()) {
                    //WebSocketSession.sendMessage(...) 的参数约束。
                    // 其 sendMessage 方法要求传入 WebSocketMessage 类型的参数。
                    //WebSocket 协议规定了 两种消息类型：文本消息（TextMessage），二进制消息（BinaryMessage）
